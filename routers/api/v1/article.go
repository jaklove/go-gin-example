...
import (
	...
	"github.com/jacklove/go-gin-example/pkg/app"
	...
	"github.com/jacklove/go-gin-example/service/article_service"
...
func DeleteArticle(c *gin.Context) {
	...
	valid := validation.Validation{}
	valid.Required(id,"id").Message("必须传入")
	valid.Min(id, 1, "id").Message("必须大于0")
	code := e.INVALID_PARAMS
	if valid.HasErrors(){
		var msg string
		for _, err := range valid.Errors {
			msg = fmt.Sprintf("%s%s",err.Key,err.Message)
			c.JSON(http.StatusOK,gin.H{
				"code": code,
				"msg":  msg,
				"data": make(map[string]string),
